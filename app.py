...
def generate_llm_recommendation(disease, input_data, risk_models):
    # Normalize input for better caching
    input_data_str = json.dumps(input_data, sort_keys=True)
    models_str = ",".join(sorted(risk_models))
    cache_key = hashlib.md5(f"{disease}_{input_data_str}_{models_str}".encode()).hexdigest()
    ...
    prompt = (
        f"Patient is likely at risk of {disease}. "
        f"Models detecting risk: {models_str}. "
        f"List 2–3 evidence-based medical steps as bullet points."
    )
    ...
    try:
        ...
        gemini_cache[cache_key] = final_output
        return final_output
...
def predict_lung():
    ...
    input_data = np.array([[age, gender, smoking, coughing, chest_pain, shortness_breath]])
